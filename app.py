...
def add_project():
    ...
    if request.method == "POST":
        ...
        new_project = Project(
            name=name, description=description, user_id=current_user.id
        )
...
def add_task(project_id):
    ...
    if project.user_id != current_user.id:
        flash(
            "You do not have permission to add tasks to this project", "danger"
        )
        return redirect(url_for("dashboard"))
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        # --- VALIDATION ---
        if not new_username or not new_email or not new_first_name \
                or not new_last_name:
            flash("All fields are required.", "danger")
            return redirect(url_for("edit_profile"))

        # Check for existing username or email,
        # but allow the current user's own.
        ...
        if existing_user_by_username:
            flash(
                "Username already exists. Please try again.", "danger"
            )
            return redirect(url_for("edit_profile"))

        if existing_user_by_email:
            flash(
                "Email already exists. Please try again.", "danger"
            )
            return redirect(url_for("edit_profile"))
